...
getIsAuth() {//check to see if the user is authenticated
...
getIsOrg() {//Get the individual status as organization or not
  return this.isOrg;
}

getIsAdmin() { //Get the individual status as admin or not 
  return this.isAdmin;
}

  getAuthStatusListener() {  //Check the users status
...
registerUser(user: any){//Register a user
  return this.http.post(`${this.api}/register`, user);
}


login(user: any) {//Log a user in 
  return this.http.post(`${this.api}/login`, user).subscribe((res: any) => {
    ...
    if(token) {
      ...
      this.saveAuthData(token, expirationDate, this.userId, this.isOrg, this.isAdmin);// executes the method saveAuthData
    }
  ...
  }, error => { //if the user is not logged in or authenticated turn the authorization status listener off
    this.authStatusListener.next(false);
  })
} 

autoAuthUser() {//Try to automatically initialize the authorization status when the app starts
  const authInformation = this.getAuthData(); //Create a variable to store the authorization data 
  if(!authInformation) { //don't return anything if there isn't any authorization information 
    return;
  }
  ...
  if(expiresIn > 0){// if the expiration time is greate than zero execute the following code
...
private saveAuthData(token: string, expirationDate: Date, userId: string, isOrg: any, isAdmin: any) {
